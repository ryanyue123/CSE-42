...
def find_sum(l: list) -> int:
    ''
    Takes nested list of integers and returns the sum
    ''
    result = 0
    for item in l:
        if type(item) == int:
            ...
            result += find_sum(item)
    return result
'''


# 5. What does the underscore before a function mean?
'''It means that the function should only be called by other functions
of that .py file.  Although it doesn't prevent people from calling the
function outside of the file, the underscore prepending the function
name is a clear sign that it should not be.
'''

# 6. What does the if __name__ == '__main__' statement mean?
'''
It means that only when the program is the original program being
called do the statements within the if statement run.  If the program is
imported from another program and run from that one, __name__ ==
'__main__' will be false.
'''
